...
同じ仕組みで、`TATEGAKI_HOST` `SYNCTHING_HOST` `OPENVPN_HOST` `EPGSTATION_HOST` を入れると、それぞれ `tategaki.<domain>` `syncthing.<domain>` `openvpn.<domain>` `epgstation.<domain>` をまとめて reverse proxy 配下に置けます。
